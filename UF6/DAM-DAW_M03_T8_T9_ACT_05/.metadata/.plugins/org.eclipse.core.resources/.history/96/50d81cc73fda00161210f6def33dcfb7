...
public class Main {
	...
	private static int menu() throws NumberFormatException, IOException {
		...
		System.out.println("[4] Grupos con más de 3 miembros");
		System.out.println("[0] Salir");

		r = Integer.parseInt(pideTexto("Elige una opción:"));
	...
	// Método para pedir Inputs al usuario
	...
	private static void CrearGrupo() throws IOException {
		...
		int numero = Integer.parseInt(pideTexto("Introduce el número de miembros"));
	...
	private static void MostrarEstilo() throws IOException {
		...
		q.descend("estilo").orderAscending();
	...
	private static void MostrarTres() {
		...
		q.descend("nIntegrantes").orderDescending();
		ObjectSet res = q.execute();
		while (res.hasNext()) {
